...
int main(int argc, char *argv[]) {
  ...
  for (int run = 0; run < nruns; run++) {
    auto perRunJson = nlohmann::json();
    char execThreadArg[64];
    for (int nThreads = 1; nThreads <= nproc; nThreads++) {
      if (fork() == 0) {
        std::snprintf(execThreadArg, sizeof(execThreadArg),
                      "--kokkos-num-threads=%d", nThreads);
        execArgv[execArgc++] = (char *)&execThreadArg;
        ...
        auto perThreadJson = nlohmann::json::parse(localOutput);
        localOutput.close();
        perRunJson.push_back(
            {{"num_threads", nThreads}, {"run_log", perThreadJson}});
      }
    }

    rootJson.push_back({{"run_id", run}, {"run_log", perRunJson}});
